...
func (h *Executor) Execute(msg *core.Message) ([]byte, error) {
	...
	// If the state implements SenderAware, inform it of the transaction sender.
	// This allows wrappers like BalancePrimingWrapper to perform sender-specific optimizations.
	if sa, ok := h.state.(SenderAware); ok {
		sa.SetSender(msg.From)
	}
