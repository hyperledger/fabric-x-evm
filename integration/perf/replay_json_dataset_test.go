...
func runReplayTest(t *testing.T, processingWorkerCount int, submittingWorkerCount int, cfg replayConfig) (float64, int64, int64) {
	...
	// Queue selection: V2 (dependency-graph aware) for closed-loop so we don't get
	// MVCC aborts; V1 (FIFO, no dependency tracking) for open-loop so we genuinely
	// push tx to the orderer as fast as we can and let the committer's MVCC handle
	// conflicts. This is the "remove our own bottleneck, see what fabric can ingest"
	// experiment.
	var txQueue gwcore.TxQueueInterface
	switch cfg.loop {
	case loopModeOpen:
		t.Logf("Open-loop: using FIFO TxQueue (V1) — no gateway-side dependency serialization")
