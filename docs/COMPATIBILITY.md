...
- **Push subscriptions**: no `eth_subscribe` / `eth_unsubscribe` yet. Polling filters for blocks
 and logs are supported; `eth_newPendingTransactionFilter` is not (no pending-tx model under
