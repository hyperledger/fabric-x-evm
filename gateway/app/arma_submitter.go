...
type armaOrdererConn struct {
	conn   *grpc.ClientConn
	client orderer.AtomicBroadcastClient
	addr   string
}

func newArmaOrdererConn(conf network.OrdererConf) (*armaOrdererConn, error) {
...
func (o *armaOrdererConn) broadcast(ctx context.Context, env *common.Envelope) error {
...
func newArmaSubmitter(config []network.OrdererConf, s sdk.Signer, logger sdk.Logger) (*armaSubmitter, error) {
	...
	orderers := make([]*armaOrdererConn, len(config))
	for i, c := range config {
		o, err := newArmaOrdererConn(c)
		if err != nil {
			return nil, err
		}
		orderers[i] = o
