...
var (
	...
	// LoadgenWorkers reports the configured worker counts for the current run.
	// `kind` is "processing" (gateway in-process worker pool) or "submitting"
	// (workload-side submitter goroutines). Surfaces config alongside throughput
	// so any chart can be read without referring back to the run log.
	LoadgenWorkers = promauto.NewGaugeVec(prometheus.GaugeOpts{
		Name: "evm_loadgen_workers",
		Help: "Configured worker count for the current run, by kind (processing|submitting)",
	}, []string{"kind"})
)
