...
// Complete marks a transaction as completed and promotes any waiting transactions
// that are no longer blocked.
// This method is idempotent - safe to call multiple times for the same transaction.
func (q *TxQueueV2) Complete(hash common.Hash) {
	...
	// Signal workers if we promoted any transactions
	if promoted {
		q.cond.Broadcast()
	}
}
...
func (q *TxQueueV2) completeUnlocked(hash common.Hash) bool {
	...
	if numBlocked > 0 {
		loggerV2.Debugf("Complete: tx %s unblocked %d txs, promoted=%v, waiting=%d ready=%d",
			hash.Hex()[:10], numBlocked, promoted, q.waitingList.Len(), q.readyList.Len())
	}
...
func (q *TxQueueV2) Handle(ctx context.Context, block *domain.Block) error {
	...
	for _, tx := range block.Transactions {
		...
		if q.completeUnlocked(txHash) {
			promoted = true
		}
	}

	// Signal workers if any transactions were promoted
	if promoted {
		q.cond.Broadcast()
	}
