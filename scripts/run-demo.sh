...
#   3. Optionally restarts the Fabric-X backend network (dectrust1-4) via Ansible
#   4. Generates testdata on dectrust5 if missing, distributes to dectrust6/7
#   5. Streams demo-staging.sh output in real time
#   6. Prints a summary
#
# Usage:
...
#   --submitting-workers N  number of parallel submitting workers per replica (default: 8)
...
if [[ "$DRY_RUN" != true ]]; then
    vrun ssh "$CTL" "
        ...
        # Symlink into the standard Ansible collections path so roles can be resolved
        # when running 'make fabric_x start/restart' from the collection directory.
    ...
    echo "+ [git clone/sync ${FORK_URL} branch ${FORK_BRANCH} at ${COLLECTION_ON_CTL}]"
    echo "+ [symlink ${COLLECTION_ON_CTL} → ~/.ansible/collections/.../hyperledger/fabricx]"
fi
...
if [[ "$RESTART_NETWORK" == true ]]; then
    ...
    if [[ "$DRY_RUN" != true ]]; then
        vlog "Running 'make fabric_x restart' via Ansible on dectrust8..."
        vrun ssh "$CTL" "
            cd '${COLLECTION_ON_CTL}'
...
[[ "$SUBMITTING_WORKERS" != "8" ]] && demo_args+=" --submitting-workers ${SUBMITTING_WORKERS}"
